...
local function doCmd(uid, cmd, args)
    ...
    if session and session.game and session.gameAgent then
        if nodeName == session.game then
            return cell.call(session.gameAgent, "doCmd", uid, cmd, args)
        else
            return cluster.call(session.game, session.gameAgent, "doCmd", uid, cmd, args)
        end
    else
        ...
        return cluster.call(gameServer.nodeName, gameAgent, "doCmd", uid, cmd, args)
    end
end

function playerUtil.getPassword(uid)
    local errcode, password = doCmd(uid, "login.getPassword")
